          ...
          <div key={role} className="mb-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {grouped[role].map((member) => (
                <div key={member.id} className="bg-gray-50 rounded-xl p-4 text-center hover:shadow-md transition">
                  {member.profileImage ? (
                    <img src={member.profileImage} alt={member.name} className="w-20 h-20 rounded-full mx-auto object-cover mb-3" />
                  ) : (
